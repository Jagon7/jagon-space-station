...
export default function AnnouncementsFeed({ announcements }: { announcements: Announcement[] }) {
  const types = useMemo(() => {
    const present = Array.from(new Set(announcements.map((a) => a.type)));
    return ["全部", ...present];
  }, [announcements]);
  const [active, setActive] = useState("全部");

  const filtered = active === "全部" ? announcements : announcements.filter((a) => a.type === active);
  ...
  return (
    <>
      ...
      <div className="rounded-xl border border-[#1e2a3a] bg-[#0d1220] overflow-hidden">
        ...
        {filtered.length === 0 ? (
          <div className="px-5 py-10 text-center text-sm text-slate-600 font-mono">
            {announcements.length === 0 ? "請等待 Actions 執行後重新整理" : "此分類目前無公告"}
          </div>
        ) : (
          <div className="divide-y divide-[#1e2a3a]/50">
            {filtered.map((a, i) => (
              <div key={i} className="px-5 py-4 hover:bg-[#1e2a3a]/30 transition-colors flex items-start gap-4">
                <span className="font-mono text-xs text-slate-600 w-24 flex-shrink-0 mt-0.5">
        ...
        <div className="px-5 py-3 border-t border-[#1e2a3a]">
          <p className="text-xs text-slate-600 font-mono">
            資料來源：公開資訊觀測站（MOPS）每日重大訊息（上市 + 上櫃）
          </p>
        </div>
      </div>
    </>
